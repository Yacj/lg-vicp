...
- 知识库为 Wiki 式层级体系：Document → Section（章节树）→ Page（页面）→ Page Block（标题/段落/表格/图片块）是知识组织与阅读层，`knowledge_chunks` 仅作辅助检索索引（不删除但不再是核心）；AI 检索走 `searchWikiHierarchy`（体系 → 文档 → 章节/页面/内容块，Chunk 辅助召回，结果带 `retrievalUnit`），小资料整节进入上下文（≤2000 字），禁止默认"全库 TopN chunk 拼接"；历史资料用 CHUNK_REBUILD/reparse 逐步升级 Wiki 层（`pnpm backfill:knowledge-wiki`）。
- AI 来源契约统一走 `src/modules/ai/ai-source.mapper.ts`（AiSourceRef），正常生成与 regenerate 复用同一 mapper，无证据时 sources 为空数组；来源详情/完整页/高亮走 `GET /api/v1/ai/knowledge/source-detail`，与公开文库共用 `knowledge-wiki-read.service`（只读 PUBLISHED+生效中，私有项目越权返回 404）。
...
- 知识库文档、版本、解析、检索：知识库模块（`src/modules/knowledge/`，含层级检索 `searchWikiHierarchy`、统一 Wiki 读取 `knowledge-wiki-read.service.ts`、C 端公开文库 `knowledge-client.routes.ts`）、`knowledge_*`/`parsing_jobs` 表及 `drizzle/` 最新迁移（knowledge_sections/knowledge_page_blocks 结构在 0025 补齐）；历史 Wiki 回填 `pnpm backfill:knowledge-wiki`；后台知识库接口按 `system:knowledge:*` 权限码授权。
