...
- `/api/v1/platform/collection/*`：B 端独立采集管理（手动采集 / 自动来源 / 任务确认入库），按 `system:collection:*` 权限码授权；采集完成进入 `WAITING_CONFIRM`，确认后导入 Knowledge 为 DRAFT，禁止自动发布。详见 `docs/collection/README.md`。
- `/api/v1/platform/masterdata/*`：Legacy 产品/材料主数据（普通菜单已隐藏，表与审核 API 保留兼容）；新资料进入 Knowledge，计算优先 VERIFIED `knowledge_facts`，缺省 fallback 本域已发布读取。企业内容/证书旧审核 API 仅兼容保留。
...
- 仅 `general_chat` 作为 C 端默认入口；用户不选择场景或系统指令。知识检索由能力路由选择允许的 Tool Set 后由 Agent 调用，只检索 PUBLISHED + AI_ENABLED + 当前用户有权访问的资料。`resolveAiCapabilities` 不再代替 Agent。快捷提问见 `docs/ai/quick-prompt.md`。其他场景 `visibility=INTERNAL`，Prompt 版本化保留给技术管理员。
- 会话滚动摘要（`ai_conversation_states`）与项目长期记忆（`project_ai_memories`）分别承担短期压缩和跨会话项目事实；ASSUMPTION / 未核实记录不得当事实注入。Agent Run 使用现有 `ai_tool_calls` 真实落库，热工计算必须走确定性引擎。
...
- 产品/材料参数主数据（Legacy）：主数据模块（`src/modules/masterdata/`），普通菜单已隐藏，表与 API 保留兼容；新功能禁止继续增加 Product 依赖。确定性参数走 `knowledge-fact-resolver.ts`（优先 VERIFIED Fact）。详见 `docs/masterdata/README.md`。
- 采集管理（独立 Domain）：`src/modules/collection/`、`collection_sources`/`collection_tasks` 表，队列 `collection-fetch`，权限码 `src/shared/collection-permissions.ts`。确认入库复用知识库 create-with-file，产物 DRAFT。详见 `docs/collection/README.md`。
