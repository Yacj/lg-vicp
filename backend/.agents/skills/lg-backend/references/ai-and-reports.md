...
报告流程：结构化 JSON -> HTML 模板 -> HTML/PDF/图片；结构化 JSON -> Word。AI 只生成草稿内容，模板决定版式，Worker 负责导出。多条 AI 回答生成报告时，使用 `report_sources` 保存来源顺序和回答快照。`reports.projectId` 可空，继承会话项目；无项目会话可生成独立报告。公开项目仅开放已发布报告。聊天图片走 `ai_message_attachments`，Vision 只注入观察上下文。
