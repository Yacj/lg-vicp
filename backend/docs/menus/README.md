# B 端菜单信息架构（2026-09：产品中心退出普通菜单 + 采集管理独立）

普通主菜单：工作台（Admin-Web 静态首页 `/`，不入库）+ 项目管理 / 知识中心 / 采集管理 / 报告管理 / AI 配置 / AI 运营 / 系统管理。
产品中心整棵子树 `visible=false`（routePath / Backend Route 保留兼容，不 DROP 表）。采集管理是独立 Domain，不是 Knowledge 子模块。
...
产品中心 /products（隐藏 Legacy，Backend API 保留）
知识中心 /knowledge
...
├─ 资料采集源 /knowledge/crawlers（隐藏 Legacy，普通入口改采集管理）
└─ 质量与调试 /knowledge/quality（AI 问答测试 / 解析异常 / 检索效果 / 高级调试）
采集管理 /collection
├─ 手动采集 /collection/manual
├─ 自动采集源 /collection/sources
...
| 产品中心 /products | 普通菜单隐藏（Legacy）；资料改走知识中心 / 采集管理 |
...
- **隐藏路由（visible=false、enabled=true）**：/ai、/thermal/calc-records、/review-center/queue、/monitor/audit、/monitor/online、/monitor/job、/monitor/cache、/ai-config/scenes、/ai-config/prompts、/reports/templates、/content/approve、/content/publish、/products（产品中心整棵目录）、/knowledge/crawlers（旧抓取源）。前端静态注册后即可访问；企业信息审核/发布仅兼容旧接口。
- **采集管理（新增一级）**：/collection、/collection/manual、/collection/sources、/collection/tasks，权限码 `system:collection:*`。
...
- Collection / Knowledge Facts 需要 drizzle migration（`collection_sources` / `collection_tasks` / `knowledge_facts` + `files.source=COLLECTION`）。
- 菜单变更重跑 `pnpm db:seed` 即幂等生效；全程单事务（先删废弃目录与早期幽灵菜单 → 先父后子 upsert 新树 → 悬空节点兜底清理），失败自动回滚。
- 产品中心叶子 routePath 不变，仅 `/products` 目录 hidden，角色权限关联零损失。

## 六、Admin-Web 同步清单

1. 普通侧边栏不再渲染产品中心（产品管理/材料参数/构造体系/节点图库/产品热工数据/材料对比配置）；对应 Backend Route 仍可用，仅技术管理员走隐藏路由或接口。
2. 新增采集管理一级：手动采集 `/collection/manual`、自动采集源 `/collection/sources`、采集任务 `/collection/tasks`。契约见 `docs/collection/README.md`。
3. 知识中心去掉「资料采集源」普通入口（`/knowledge/crawlers` 隐藏）。
4. 静态注册隐藏路由（随页面实现逐个开启）：`/review-center/queue`、`/ai`、`/thermal/calc-records`、`/monitor/audit|online|job|cache`。
5. 系统监控页面（views/monitor/**）当前缺失，实现后由菜单管理开启 visible。
6. 文件中心页面实现后，在知识中心下新增 `/files` 菜单（permissionCode `file:center:view`；本次未 seed，避免不可用页面）。
7. 页面内写死的标题/面包屑与新菜单名对齐：模板报告→报告列表、图集参考表→图集热工参考表、材料库→保温材料库、材料参数版本→材料性能参数、节点图纸→节点大样图、对比版本→对比规则、标准文档→标准文件、指标管理→节能指标、替代关系→新旧标准替代、采集来源→标准采集源、企业证书→企业资质、审计日志→操作日志、AI 运营→AI 运行情况、报告中心→报告管理。
8. 工作台"我的待办 / 待审核"需要 Dashboard 聚合 API（本次未实现，建议 `GET /api/v1/platform/dashboard/summary`：待审数、最近项目、最近报告、资料异常、AI 用量概览）。
9. AI 配置 / AI 运营页面已由静态路由承载（/ai-config/*、/ai-ops/*），后端菜单同名路径直接复用静态页面；工作台首页不再提供「AI 与系统」聚合面板，AI 入口经侧栏一级菜单进入；报告管理内嵌"报告审核"入口。
10. 企业信息普通页对接 `/api/v1/platform/company/profile` 与 `/qualifications`（见 `docs/company/README.md`），不要再走 `/platform/masterdata/enterprise-*` 审核流；不要在报告设置里维护第二套企业名称/Logo。C 端关于我们用 `GET /api/v1/company/about`。

## 七、AI 辅助录入边界（本次信息架构调整配套约定）

- 标准规范：上传/采集 → AI 提取（地区/建筑类型/部位/指标/限值/生效日期/替代标准）→ 结构化草稿 → 人工校对 → 审核 → 发布 → 热工/推荐消费正式数据。AI 提取结果不得直接进入正式计算。
- 产品/图集：正式资料进入 Knowledge；AI 辅助抽取的确定性参数写入内部 `knowledge_facts`（必须带 source），不得做成新的 B 端产品中心。计算优先 VERIFIED Fact，缺省 fallback Legacy 产品表。
- 材料对比：AI 可从已审核知识资料辅助生成对比规则草稿与证据引用；项目正式材料对比仍消费已审核 + 已发布的 comparison 数据（本轮暂不强制迁移）。
