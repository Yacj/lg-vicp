...
种子单一事实源：`src/shared/thermal-permissions.ts`（`THERMAL_PERMISSIONS` + `THERMAL_PERMISSION_SEEDS`），合并进 `src/db/seed.ts` 幂等写入并挂菜单"图集热工参考表"（routePath `/thermal`、permissionCode `system:thermal:list`）。SUPER_ADMIN 直通。
