...
export const errorHandlerPlugin = fp(async (app) => {
  app.setErrorHandler((error, request, reply) => {
    if (error instanceof AppError) {
      return reply.status(error.statusCode).send(fail(request.id, error.code, error.message));
    }

    if (error instanceof ZodError) {
      const message = error.issues.map((issue) => issue.message).join("；");
      return reply.status(400).send(fail(request.id, "VALIDATION_ERROR", message));
    }
    ...
    if (databaseError.code === "23505") {
      return reply.status(409).send(fail(request.id, "CONFLICT", "数据已存在，请勿重复提交"));
    }
    if (databaseError.code === "23503") {
      return reply.status(400).send(fail(request.id, "REFERENCE_INVALID", "关联的数据不存在或已失效"));
    }
