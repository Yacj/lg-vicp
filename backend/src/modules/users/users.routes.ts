...
async function requireActiveUser(app: FastifyInstance, request: Parameters<typeof getCurrentUser>[0], targetUserId: string) {
  assertUserInScope(request, targetUserId);
  const [user] = await app.db.select({ id: users.id, role: users.role }).from(users).where(and(eq(users.id, targetUserId), isNull(users.deletedAt))).limit(1);
  if (!user) throw new NotFoundError("用户不存在");
  assertCanManageSuperAdmin(request, user.role);
  return user;
}
...
async function validateAssignments(
  ...
  if (departmentIds !== undefined) {
    ...
    await assertDepartmentIdsWithinActor(app, getCurrentUser(request), departmentIds);
  }
  if (postIds !== undefined) {
    await assertPermission(request, "system:user:post");
    const rows = postIds.length === 0 ? [] : await app.db.select({ id: posts.id, enabled: posts.enabled }).from(posts).where(inArray(posts.id, postIds));
    ...
    await assertPostIdsAssignable(app, getCurrentUser(request), postIds);
  }
...
export async function userRoutes(app: FastifyInstance) {
  ...
  route.get("/users", { preHandler: [app.authenticate], schema: { tags: ["B端 / 平台 / 用户管理"], summary: "获取用户列表（支持包含下级部门筛选）", querystring: listQuerySchema } }, async (request) => {
    ...
    const keywordFilter = request.query.keyword ? (() => {
  ...
  route.post("/users/import", { preHandler: [app.authenticate], schema: { tags: ["B端 / 平台 / 用户管理"], summary: "导入用户 CSV", body: importBodySchema } }, async (request) => {
    ...
    if (actor.accessibleUserIds !== null) throw new ForbiddenError("当前数据范围不支持批量导入账号");
      ...
      await app.db.transaction(async (tx) => {
        for (const [index, line] of lines.entries()) {
          ...
          try {
            ...
            const phone = parsed.data.phone ? normalizePhone(parsed.data.phone) : (isPhone ? identifier : undefined);
            await tx.transaction(async (rowTx) => {
              const [user] = await rowTx.insert(users).values({
            ...
            errors.push({ row: index + 2, message: error instanceof ConflictError ? error.message : "用户名、手机号或邮箱已存在" });
          }
        }
  ...
  route.post("/users", { preHandler: [app.authenticate], schema: { tags: ["B端 / 平台 / 用户管理"], summary: "创建用户", body: createUserBodySchema } }, async (request) => {
    ...
    try {
      ...
      const [identityTaken] = await tx.select({ id: userIdentities.id }).from(userIdentities).innerJoin(users, eq(users.id, userIdentities.userId)).where(and(eq(userIdentities.identifier, identifier), isNull(users.deletedAt), isNull(userIdentities.deletedAt))).limit(1);
      ...
      await writeAuditLog({ db: tx, request, actor, action: AUDIT_ACTIONS.USER_CREATED, targetType: "user", targetId: user!.id, afterJson: { ...user, identifier, departmentIds: assignments.departmentIds ?? [], postIds: assignments.postIds ?? [], roleIds: assignments.roleIds ?? [] } });
      return user!;
      });
    } catch (error) {
  ...
  route.patch("/users/:id/status", { preHandler: [app.authenticate], schema: { tags: ["B端 / 平台 / 用户管理"], summary: "修改用户状态", params: userParamsSchema, body: updateStatusBodySchema } }, async (request) => {
    ...
    const [before] = await app.db.select().from(users).where(and(eq(users.id, request.params.id), isNull(users.deletedAt))).limit(1);
    if (!before) throw new NotFoundError("用户不存在");
    assertCanManageSuperAdmin(request, before.role);
    const [updated] = await app.db.transaction(async (tx) => {
  ...
  route.post("/users/:id/restore", { preHandler: [app.authenticate], schema: { tags: ["B端 / 平台 / 用户管理"], summary: "恢复用户", params: userParamsSchema } }, async (request) => {
    ...
    const identityRows = await app.db.select({ id: userIdentities.id, identifier: userIdentities.identifier }).from(userIdentities).where(eq(userIdentities.userId, before.id));
    for (const identity of identityRows) {
      const normalizedIdentifier = normalizeLoginIdentifier(identity.identifier);
      const [sameIdentifier] = await app.db.select({ id: userIdentities.id }).from(userIdentities).innerJoin(users, eq(users.id, userIdentities.userId)).where(and(sql`case when btrim(${userIdentities.identifier}) ~ '^\\+?[0-9]{6,20}$' then btrim(${userIdentities.identifier}) else lower(btrim(${userIdentities.identifier})) end = ${normalizedIdentifier}`, isNull(userIdentities.deletedAt), isNull(users.deletedAt), ne(users.id, before.id))).limit(1);
      if (sameIdentifier) throw new ConflictError("该登录账号已被其他用户使用，无法恢复");
    }
  ...
  route.post("/users/:id/reset-password", { preHandler: [app.authenticate], schema: { tags: ["B端 / 平台 / 用户管理"], summary: "重置用户密码", params: userParamsSchema, body: resetPasswordSchema } }, async (request) => {
    ...
    assertUserInScope(request, request.params.id);
    const [target] = await app.db.select({ role: users.role }).from(users).where(eq(users.id, request.params.id)).limit(1);
    if (!target) throw new NotFoundError("用户不存在");
    assertCanManageSuperAdmin(request, target.role);
    ...
    await app.db.transaction(async (tx) => {
      const passwordHash = await argon2.hash(request.body.password, { type: argon2.argon2id });
      await tx.update(userIdentities).set({ passwordHash, updatedAt: new Date() }).where(eq(userIdentities.id, identity.id));
      await writeAuditLog({ db: tx, request, actor, action: AUDIT_ACTIONS.USER_PASSWORD_RESET, targetType: "user", targetId: request.params.id });
    });
  ...
  route.put("/users/:id/posts", { preHandler: [app.authenticate], schema: { tags: ["B端 / 平台 / 用户管理"], summary: "分配用户岗位", params: userParamsSchema, body: idsBodySchema } }, async (request) => {
    ...
    const postIds = [...new Set(request.body.ids)];
    await assertPostIdsAssignable(app, actor, postIds);
    const existing = postIds.length ? await app.db.select({ id: posts.id, enabled: posts.enabled }).from(posts).where(inArray(posts.id, postIds)) : [];
    if (existing.length !== postIds.length) throw new NotFoundError("部分岗位不存在");
    if (existing.some((item) => !item.enabled)) throw new ForbiddenError("不能绑定已停用的岗位");
    await app.db.transaction(async (tx) => {
      const before = await tx.select({ postId: userPosts.postId }).from(userPosts).where(eq(userPosts.userId, user.id));
      await tx.delete(userPosts).where(eq(userPosts.userId, user.id));
      if (postIds.length) await tx.insert(userPosts).values(postIds.map((postId) => ({ userId: user.id, postId })));
      await writeAuditLog({ db: tx, request, actor, action: AUDIT_ACTIONS.USER_POST_CHANGED, targetType: "user", targetId: user.id, beforeJson: { postIds: before.map((item) => item.postId) }, afterJson: { postIds } });
    });
  ...
  route.put("/users/:id/departments", { preHandler: [app.authenticate], schema: { tags: ["B端 / 平台 / 用户管理"], summary: "分配用户部门", params: userParamsSchema, body: idsBodySchema } }, async (request) => {
    ...
    if (existing.length !== departmentIds.length) throw new NotFoundError("部分部门不存在或已删除");
    await app.db.transaction(async (tx) => {
      const before = await tx.select({ departmentId: userDepartments.departmentId }).from(userDepartments).where(eq(userDepartments.userId, user.id));
      await tx.delete(userDepartments).where(eq(userDepartments.userId, user.id));
      if (departmentIds.length) await tx.insert(userDepartments).values(departmentIds.map((departmentId, index) => ({ userId: user.id, departmentId, isPrimary: index === 0 })));
      await writeAuditLog({ db: tx, request, actor, action: AUDIT_ACTIONS.USER_DEPARTMENT_CHANGED, targetType: "user", targetId: user.id, beforeJson: { departmentIds: before.map((item) => item.departmentId) }, afterJson: { departmentIds } });
    });
