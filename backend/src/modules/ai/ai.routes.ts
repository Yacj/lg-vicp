...
export async function aiRoutes(app: FastifyInstance) {
  ...
  const knowledgeQaBodySchema = z.object({
  ...
  route.post("/knowledge-qa", {
    ...
    schema: {
      ...
      body: knowledgeQaBodySchema
    }
  }, async (request, reply) => {
    const user = getCurrentUser(request);
    // 先执行真实检索（仅 PUBLISHED 版本 + ACTIVE 文档，与 /search 同一查询管线），
    // 检索结果注入提示词并随 done.sources 返回引用标注
    const chunks = await runSearch(app, request.body.query, {
      projectId: null,
    ...
    // 创建知识问答会话（B 端场景，标题取问题前 40 字），随后走标准生成链路
    const conversation = await app.db.transaction(async (tx) => {
      const [created] = await tx.insert(aiConversations).values({
        userId: user.id,
        projectId: null,
        ...
        title: request.body.query.slice(0, 40),
        reasoningMode: request.body.reasoningMode
      }).returning();
      await writeAuditLog({
        db: tx, request, actor: user,
        action: AUDIT_ACTIONS.AI_CONVERSATION_CREATED,
        targetType: "ai_conversation", targetId: created!.id, afterJson: created
      });
      return created!;
