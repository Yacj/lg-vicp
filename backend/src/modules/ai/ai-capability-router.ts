...
const PROJECT_PATTERN = /当前项目|本项目|这个项目|项目资料|项目里|我的项目/;
...
export function resolveAiCapabilities(input: ResolveAiCapabilitiesInput): AiCapabilities {
  ...
  const askedAboutProject = PROJECT_PATTERN.test(message);
  const needProjectContext = Boolean(input.projectId) || askedAboutProject;
