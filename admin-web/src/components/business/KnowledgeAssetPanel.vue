...
<template>
  <div class="knowledge-assets">
    <t-alert v-if="noTextLayer" theme="warning" title="未检测到可搜索文字">
      该文件为转曲/扫描型 PDF，未检测到可搜索文字。可绑定可搜索文本版、启用 OCR（如果后端开放），或仅作为原文浏览。
    </t-alert>
    <div class="knowledge-assets__grid">
      <article v-for="item in [{ role: 'ORIGINAL' as const, label: '正式原文件', purpose: '用户查看 / 对外引用', asset: original }, { role: 'SEARCH_SOURCE' as const, label: '检索文件', purpose: 'AI 查询', asset: searchSource }]" :key="item.role" class="knowledge-assets__card">
        <div class="knowledge-assets__head"><strong>{{ item.label }}</strong><t-tag size="small" variant="outline" :theme="item.asset ? 'success' : 'warning'">{{ item.asset ? '已上传' : '未准备' }}</t-tag></div>
        <div class="knowledge-assets__name">{{ item.asset?.fileName ?? '当前没有文件' }}</div>
        <div class="knowledge-assets__purpose">用途：{{ item.purpose }}<template v-if="item.role === 'SEARCH_SOURCE' && searchSource === original && original">（当前复用正式原文件）</template></div>
    ...
    <div class="knowledge-assets__mode">
      ...
      <t-select
        :value="usageMode"
        :disabled="!editable"
        :options="[{ label: 'AI 检索', value: 'AI_ENABLED' }, { label: '仅原文浏览', value: 'BROWSE_ONLY' }]"
        style="width: 160px"
        @change="(value: unknown) => emit('usageModeChange', value as KnowledgeUsageMode)"
      />
    </div>
    <t-alert v-if="usageMode === 'BROWSE_ONLY'" theme="info" message="当前用途为仅原文浏览，此文档不会进入 AI 检索。" />
  </div>
</template>
