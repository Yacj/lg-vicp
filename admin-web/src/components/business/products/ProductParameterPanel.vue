...
/**
 * 产品参数列表面板（产品管理工作区 Tab 内容区）。
 * 传入 lockedSpecId 时锁定为单一规格视图（产品详情工作台内使用）。
 */
...
const list = useCrudList<ProductParameter, ProductParameterQuery>({
  createQuery: () => ({ page: 1, pageSize: 20, keyword: '', status: '', specId: props.lockedSpecId ?? '', parameterCode: '', paramSource: '' }),
...
watch(() => props.lockedSpecId, (value) => {
  if (value && list.query.specId !== value) {
    list.query.specId = value
    void list.refresh()
  }
})
...
<template>
  <div class="vicp-param-panel">
    <AppSearchPanel :loading="list.isLoading.value" @reset="list.reset" @search="list.search">
      ...
      <t-form-item v-if="!lockedSpecId" label="产品规格">
        <t-select
