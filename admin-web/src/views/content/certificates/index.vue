...
import {
  createEnterpriseCertificate,
  deleteEnterpriseCertificate,
  fetchEnterpriseCertificates,
  runEnterpriseCertificateWorkflow,
  updateEnterpriseCertificate,
} from '@/api/modules/masterdata'
import type { AppTableAction } from '@/types/crud'
import type { EnterpriseCertificate, EnterpriseCertificateInput, EnterpriseCertificateQuery } from '@/types/masterdata'
...
const list = useCrudList<EnterpriseCertificate, EnterpriseCertificateQuery>({
  createQuery: () => ({ page: 1, pageSize: 20, keyword: '', status: '' }),
  fetcher: ({ query, page, pageSize, signal }) =>
    fetchEnterpriseCertificates({ ...query, page, pageSize, status: !query.status || query.status === 'all' ? undefined : query.status } as EnterpriseCertificateQuery, signal),
  immediate: true,
  rowKey: 'id',
})

const drawer = useCrudDrawer<EnterpriseCertificateInput, EnterpriseCertificate>({
  createForm: () => ({
    certName: '',
    certNo: '',
    issuer: '',
  ...
  editForm: (entity) => ({
    certName: entity.certName,
    certNo: entity.certNo ?? '',
  ...
  submit: async ({ mode, data, entity }) => {
    ...
    const result = mode === 'create'
      ? await createEnterpriseCertificate(input)
      : await updateEnterpriseCertificate(entity!.id, input)
    return result
  },
  onSuccess: () => list.refresh(),
})

const deleteAction = useConfirmedCrudAction<EnterpriseCertificate, unknown>({
  action: (row) => deleteEnterpriseCertificate(row.id),
  confirm: (row) => ({ title: '删除草稿', content: `确定删除「${row.certName}」？仅草稿可删除。`, danger: true }),
  successMessage: '已删除',
  onSuccess: () => list.refresh(),
})

const workflow = useWorkflowActions<EnterpriseCertificate>({
  entityName: '企业证书',
  run: (id, action) => runEnterpriseCertificateWorkflow(id, action).then((r) => r.item),
  onSuccess: () => list.refresh(),
})
...
const columns: PrimaryTableCol<TableRowData>[] = [
  { cell: (_, { row }) => h('div', [
    h('div', { class: 'vicp-cert-name' }, row.certName),
    row.certNo ? h('div', { class: 'vicp-cert-no' }, row.certNo) : null,
  ]), colKey: 'certName', minWidth: 200, title: '证书名称 / 编号' },
  { cell: (_, { row }) => row.issuer ?? '—', colKey: 'issuer', minWidth: 140, title: '签发机构' },
  { cell: (_, { row }) => `${formatDay(row.issueDate)} ~ ${formatDay(row.expiryDate)}`, colKey: 'expiryDate', minWidth: 200, title: '有效期' },
...
function getActions(row: TableRowData): AppTableAction[] {
  const entity = row as EnterpriseCertificate
...
<template>
  <AppPage title="企业证书" description="维护企业资质证书；非版本化实体，审核发布后生效。">
    <template #search>
      <AppSearchPanel :loading="list.isLoading.value" @reset="list.reset" @search="list.search">
        <t-form-item label="关键词">
          <t-input v-model="list.query.keyword" clearable placeholder="证书名称 / 编号 / 机构" />
        </t-form-item>
    ...
    <AppCrudFormDialog
      ...
      :title="drawer.mode.value === 'create' ? '新增企业证书' : '编辑企业证书'"
    ...
    >
      <t-form-item class="vicp-form-wide" label="证书名称" name="certName">
        <t-input v-model="drawer.formData.certName" maxlength="160" placeholder="请输入证书名称" />
      </t-form-item>
      <t-form-item label="证书编号" name="certNo">
        <t-input v-model="drawer.formData.certNo" maxlength="120" placeholder="选填" />
      </t-form-item>
      <t-form-item label="签发机构" name="issuer">
        <t-input v-model="drawer.formData.issuer" maxlength="160" placeholder="选填" />
      </t-form-item>
      <t-form-item label="签发日期" name="issueDate">
        <t-date-picker v-model="drawer.formData.issueDate" clearable placeholder="选填" />
      </t-form-item>
      <t-form-item label="有效期至" name="expiryDate">
        <t-date-picker v-model="drawer.formData.expiryDate" clearable placeholder="选填" />
      </t-form-item>
      ...
      <t-form-item label="资料出处" name="evidenceSource">
        <t-input v-model="drawer.formData.evidenceSource" maxlength="500" placeholder="如：证书原件扫描件" />
      </t-form-item>
      ...
      <t-form-item class="vicp-form-wide" label="备注" name="description">
        <t-textarea v-model="drawer.formData.description" :autosize="{ minRows: 2, maxRows: 4 }" maxlength="1000" placeholder="选填" />
      </t-form-item>
    </AppCrudFormDialog>
