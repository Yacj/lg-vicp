...
const list = useCrudList<ProductAttachment, ProductAttachmentQuery>({
  createQuery: () => ({ page: 1, pageSize: 20, keyword: '', status: '', targetType: '', targetId: '' }),
  fetcher: ({ query, page, pageSize, signal }) =>
    fetchProductAttachments({ ...query, page, pageSize, status: !query.status || query.status === 'all' ? undefined : query.status, targetType: !query.targetType || query.targetType === 'all' ? undefined : query.targetType } as ProductAttachmentQuery, signal),
  immediate: true,
  rowKey: 'id',
})
...
const columns: PrimaryTableCol<TableRowData>[] = [
  ...
  { cell: (_, { row }) => row.targetId, colKey: 'targetId', minWidth: 220, title: '对象 ID' },
...
<template>
  <AppPage title="产品附件" description="产品系列 / 规格 / 企业维度的文件附件；非版本化实体，审核发布后对外可见。">
    <template #search>
    ...
    <AppCrudFormDialog
      ...
      :title="drawer.mode.value === 'create' ? '新增产品附件' : '编辑产品附件'"
    ...
    >
      <t-form-item label="挂载对象" name="targetType" required-mark>
        <t-select v-model="drawer.formData.targetType" :options="targetTypeOptions" />
      </t-form-item>
      <t-form-item label="对象 ID" name="targetId" required-mark>
        <t-input v-model="drawer.formData.targetId" placeholder="目标实体 UUID" />
      </t-form-item>
