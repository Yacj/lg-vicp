...
import {
  ...
  runComparisonVersionWorkflow,
  updateComparisonVersion,
  validateComparisonVersion,
} from '@/api/modules/comparison'
import type { AppTableAction } from '@/types/crud'
import type { ComparisonVersion, ComparisonVersionInput, ComparisonVersionQuery } from '@/types/comparison'
import type { ValidationResult } from '@/types/comparison'
import { formatDate } from '@/utils/day'
import { evidenceLevelLabels, mdReviewStatusMetaFor } from '@/utils/professional-status'

const { canAccess } = usePermissionAccess()
const canAdd = computed(() => canAccess({ permissions: ['system:comparison:add'] }))
const canEdit = computed(() => canAccess({ permissions: ['system:comparison:edit'] }))
const canRemove = computed(() => canAccess({ permissions: ['system:comparison:remove'] }))
const canApprove = computed(() => canAccess({ permissions: ['system:comparison:approve'] }))
const canPublish = computed(() => canAccess({ permissions: ['system:comparison:publish'] }))
const canList = computed(() => canAccess({ permissions: ['system:comparison:list'] }))

const feedback = useAppFeedback()

const list = useCrudList<ComparisonVersion, ComparisonVersionQuery>({
  createQuery: () => ({ page: 1, pageSize: 20, keyword: '', status: '' }),
  fetcher: ({ query, page, pageSize, signal }) =>
    fetchComparisonVersions({ ...query, page, pageSize, status: !query.status || query.status === 'all' ? undefined : query.status } as ComparisonVersionQuery, signal),
  immediate: true,
  rowKey: 'id',
})

const drawer = useCrudDrawer<ComparisonVersionInput, ComparisonVersion>({
  createForm: () => ({
    code: '',
    name: '',
    description: '',
  ...
  submit: async ({ mode, data, entity }) => {
    ...
    const result = mode === 'create'
      ? await createComparisonVersion(input)
      : await updateComparisonVersion(entity!.id, input)
    return result
  },
  onSuccess: () => list.refresh(),
})

const deleteAction = useConfirmedCrudAction<ComparisonVersion, unknown>({
  action: (row) => deleteComparisonVersion(row.id),
  confirm: (row) => ({ title: '删除草稿', content: `确定删除「${row.name}」？仅草稿可删除。`, danger: true }),
  successMessage: '已删除',
  onSuccess: () => list.refresh(),
})

const workflow = useWorkflowActions<ComparisonVersion>({
  entityName: '对比版本',
  run: (id, action) => runComparisonVersionWorkflow(id, action).then((r) => r.item),
  onSuccess: () => list.refresh(),
})
...
async function runValidate(entity: ComparisonVersion): Promise<void> {
  validateState.label = entity.name
  validateState.loading = true
  validateState.result = null
  validateState.visible = true
  try {
    const result = await validateComparisonVersion(entity.id)
...
const columns: PrimaryTableCol<TableRowData>[] = [
  { cell: (_, { row }) => h('div', [
    h('div', { class: 'vicp-cmp-name' }, row.name),
    row.code ? h('div', { class: 'vicp-cmp-code' }, row.code) : null,
...
function getActions(row: TableRowData): AppTableAction[] {
  const entity = row as ComparisonVersion
...
<template>
  <AppPage title="对比版本" description="材料对比规则的唯一审核/发布单元；对比材料、维度规则与资料出处随版本一起管理。">
    ...
    <AppCrudFormDialog
      ...
      :title="drawer.mode.value === 'create' ? '新增对比版本' : '编辑对比版本'"
    ...
    >
      <t-form-item label="版本编码" name="code" required-mark>
