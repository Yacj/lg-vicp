 ...
 * @param onProgress - 可选的回调函数，用于跟踪下载进度。
 * 该函数接收一个参数，表示下载进度百分比（0-100）。
 * @returns 返回一个 Promise，下载成功时会 resolve。
 * @throws 如果下载失败，会抛出一个错误。
 *
 * @example
 ...
 */
export async function downloadFile(url: string, onProgress?: (progress: number) => void): Promise<void> {
  try {
    const response = await axios.get(url, {
      responseType: 'blob',
      onDownloadProgress: (progressEvent) => {
        if (typeof onProgress === 'function' && progressEvent.total && progressEvent.total > 0) {
 ...
 * @private
 */
function getFileNameFromHeaders(headers: any, url: string): string {
