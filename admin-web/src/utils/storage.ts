 ...
 * 获取完整的键名（带前缀）
 */
function getFullKey(key: string, prefix = DEFAULT_PREFIX): string {
  return `${prefix}${key}`
}
 ...
 * 计算过期时间戳
 */
function calculateExpiration(days = DEFAULT_EXPIRATION_DAYS): number {
 ...
 * @param prefix 前缀（可选）
 */
export function removeStorage(key: string, prefix = DEFAULT_PREFIX): void {
  try {
    const fullKey = getFullKey(key, prefix)
 ...
 * @param prefix 前缀（可选）
 */
export function hasStorage(key: string, prefix = DEFAULT_PREFIX): boolean {
  try {
    const fullKey = getFullKey(key, prefix)
    const item = localStorage.getItem(fullKey)
 ...
 * @param prefix 前缀（可选）
 */
export function getAllStorageKeys(prefix = DEFAULT_PREFIX): string[] {
  try {
